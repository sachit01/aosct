...
namespace ATP
{
  namespace Supv
  {
    ...
    * addSupvSpeedAndPrimaryTargets
    ******************************************************************************/
    void AbstractTargetCalculation::addSupvSpeedAndPrimaryTargets() const
    {
      static uint32_t beginCp = 0U; // Must be initialized to 0
      static uint32_t endCp = 0U; // Must be initialized to 0
        ...
        it != targets->getConstMATargetIterEnd(); ++it)
      {
        ...
        if (target->getTargetType() == DS::BaseTarget::PrimaryTarget)
        {
          ...
          priTarg->addRelatedSuperviseTargets();
        }
    ...
    ******************************************************************************/
    void AbstractTargetCalculation::updateGradientAndCeilingSpeed(const uint32_t maxTrainSpeed)
    {
      ...
      OdoPosition totalSbMargin =
        static_cast<OdoPosition>(AbstractConfig::corePtr()->getSbToEbMargin()) +
        static_cast<OdoPosition>(AbstractConfig::corePtr()->getEbMargin());
      ...
      while (supTargetItr != DS::AbstractTargets::corePtr()->getSupervisedTargetIterEnd())
      {
        ...
        if (sTarget->getDirection() == suppTravelDir)
        {
          ...
          * OR in case of MA extension if target are ahead of total margin (EB + SB margin) from the MA start,
          *   it will be treated as new target and CS will be recalculated
          ...
          * CS target can have supervised targets behind MA start.*/
          ...
          const bool isNewSupvTarget = (suppTravelDir == DirForward) ?
            ((sTarget->getParentTarget()->getOdometer() >= maStartPos) || (supervisedTargetOdo >= (maStartPos - totalSbMargin))) :
            ((sTarget->getParentTarget()->getOdometer() <= maStartPos) || (supervisedTargetOdo <= (maStartPos + totalSbMargin)));
      ...
      while (supTargetItr != DS::AbstractTargets::corePtr()->getSupervisedTargetIterEnd())
      {
        ...
        if(sTarget->getDirection() == suppTravelDir)
        {
          ...
          bool isCSMATargetPassed = ((*csMATargetItr)->getDirection() == DirForward)?
                                  ((*csMATargetItr)->getOdometer() <= supvTargetOdo):
                                  ((*csMATargetItr)->getOdometer() >= supvTargetOdo);

          if(isCSMATargetPassed)
          {
            ...
            removeCsFromVector(fwVector, (*csMATargetItr)->getTargetId());
            removeCsFromVector(swVector, (*csMATargetItr)->getTargetId());
            removeCsFromVector(sbVector, (*csMATargetItr)->getTargetId());
            removeCsFromVector(ebVector, (*csMATargetItr)->getTargetId());

            //update the ceiling speed.
            currCeilingSpeed = (*csMATargetItr)->getSpeedChange();

            // find the next CS target which reduces the speed.
            csMATargetItr = findNextCSTarget(csMATargetItr, supvTargetOdo,(*csMATargetItr)->getSpeedChange());
    ...
    /******************************************************************************
    * updateWarningSpeedsForTargets
    ******************************************************************************/
    void AbstractTargetCalculation::updateCurveSpeedsForTargets() const
    {
      static uint32_t beginCp = 0U; // Must be initialized to 0
      static uint32_t endCp = 0U; // Must be initialized to 0
          ...
          if ((stDir == sTarget->getDirection()))
          {
            if (DS::BaseTarget::SupervisedTarget == sTarget->getTargetType()) // Do not set and use permitted speed for TrackDataItemTarget
            {
              ...
              else
              {
                ...
                int32_t gradient = sTarget->getGradient();
              ...
              */
                ...
                int32_t deceleration = bc.calcDeceleration(brakeability, gradient);

                curveSpeed = bc.calcFirstWarningCurveSpeed(sTarget->getOdometer(), stDir, gradient,
    ...
    ******************************************************************************/
    DS::SupervisedTarget* AbstractTargetCalculation::getMostRestrictiveTarget() const
    {
      ...
      while((it != DS::AbstractTargets::corePtr()->getSupervisedTargetIterEnd()) && (!targFound))
      {
        ...
        if (tDir == supvTarget->getDirection())
        {
