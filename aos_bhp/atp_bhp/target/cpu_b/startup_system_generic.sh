...
#
# Description:
# Startup CPU-B in generic Lab Setup
...
VFW_CMD="/opt/vfw/run/vfwChanneld -A 20 -O 5 -I 40 -C 35 -c 100 server@CoHP-C server@CoHP-A"
VIOH_CMD="/opt/run/vioserver $CYCLE 1 1 $UNIQUE_ID b"
SDP_CMD="/opt/run/SDP_B $UNIQUE_ID"
APPLICATION="/opt/bin/aos/atp_bhp_$1 -b"

# vioh log. Must be in ramdisk (/tmp)
VIOH_LOG="/tmp/vioh_b.log"
...
# Update the time to 1 Jan 1970 00:00:00
/opt/bin/aos/settimeofday -t 0

ping -W 5 -c 10 CoHP-A > /dev/null
if [ $? != 0 ]
then
    logger "Problems with the network, CoHP-A not responding!"
else
    logger "Ping to CoHP-A OK!"
fi
...
if [ "$1" == "vsim" ]
then
  # Start the time sync client for CoHP-B
  logger "Starting TimeSyncServer"
  /opt/bin/aos/TimeSyncServer -b -n $UNIQUE_ID  > /tmp/timesync.log 2>&1 &
fi
