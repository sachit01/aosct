...
TColorButton = class(TCustomControl)
  private
    ...
    procedure SetColorH(Value: TColor);
  ...
  published
    ...
    Property Highlight : TColor read FColorH write SetColorH default clBlue;
    Property shadow : TColor read FColorS write SetColorS default clNavy;
    property Cursor;
    property DragCursor;
    property DragMode;
    property Enabled;
...
function GetHighlightColor(BaseColor,high: TColor): TColor;
function GetShadowColor(BaseColor,shadow: TColor): TColor;
...
procedure Register;
begin
  RegisterComponents('MMI', [TColorButton]);
end;
...
begin
     ...
     R:=Smallest(GetRValue(ColorToRGB(BaseColor)) + 64, 255);
     G:=Smallest(GetGValue(ColorToRGB(BaseColor)) + 64, 255);
     B:=Smallest(GetBValue(ColorToRGB(BaseColor)) + 64, 255);
...
Var r,g,b :integer ;
begin
     R:= Largest(GetRValue(ColorToRGB(BaseColor)) - 64, 0);
     G:= Largest(GetGValue(ColorToRGB(BaseColor)) - 64, 0);
     B:= Largest(GetBValue(ColorToRGB(BaseColor)) - 64, 0);
...
begin
  ...
  begin
  	// Clear the background
        ...
        begin
      	clHigh := GetShadowColor(FColor,FColorS);
        clLow := GetHighlightColor(FColor,FColorH);
        end
        else
        begin
      	clHigh := GetHighlightColor(FColor,FColorH);
        clLow := GetShadowColor(FColor,FColorS);
      end;
    ...
    begin
      ...
      begin
      	Font.Color := GetHighlightColor(FColor,FColorH); DrawCaption(1);
        Font.Color := GetShadowColor(FColor,FColorS); DrawCaption(0);
...
procedure TColorButton.SetColorH(Value: TColor);
begin
     FcolorH := Value;
     Repaint;
end;
